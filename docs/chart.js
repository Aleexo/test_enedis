function createGraphFrom(nomFichierJson, getTypeNoeud, getColorChoice, getNodeColor, getLinkColor, linkToolTip, nodeToolTip, needUseLabel) {
    ...
    d3.json(`https://raw.githubusercontent.com/Aleexo/test_enedis/main/json/${nomFichierJson}`, function (error, graph) {
        ...
        const colorChoice = getColorChoice(typeNoeud)
        ...
        .force("link", d3.forceLink().id(function (d) { return d.id; }).distance(80).strength(0.05))
        .force("charge", d3.forceManyBody().strength(-30)) //Charge strength is here
        ...
        const node = svg.append("g")
            ...
            .attr("r", function (d) {
                ...
                const minRadius = 6;
                return minRadius + Math.log2(d.weight) / Math.log(2.5);
            })
            .attr("fill", nodeColor)
            .on('mouseover.tooltip', function (d) {
                tooltip.transition()
